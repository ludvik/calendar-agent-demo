...
class RescheduleStrategy(BaseModel):
    ...
    window_days: Optional[int] = Field(
        None, 
        description="Number of days to look ahead for available slots"
    )
    preferred_hours: Optional[List[int]] = Field(
        None, 
        description="List of preferred hours (9-17) to try first"
    )
...
class ConflictResolutionStrategies(BaseModel):
    ...
    by_type: Optional[TypeBasedStrategies] = Field(
        None, 
        description="Type-based conflict resolution strategies"
    )
    by_priority: Optional[bool] = Field(
        None, 
        description="Whether to use priority-based resolution"
    )
